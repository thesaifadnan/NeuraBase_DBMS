...
selectQuery Parser::parseSelect(const std::vector<Token>& tokens) {
    selectQuery q;
    size_t i = 0;
    size_t len = tokens.size();

    if (tokens[i].value != "SELECT" || tokens[i].type != TokenType::KEYWORD) {
    ...
    if (i < len && tokens[i].type == TokenType::SYMBOL && tokens[i].value == "*") {
        ...
        while (i < len && !(tokens[i].type == TokenType::KEYWORD && tokens[i].value == "FROM")) {
            if (tokens[i].type == TokenType::IDENTIFIER) {
                q.columns.push_back({tokens[i++]});
            } else if (tokens[i].type == TokenType::SYMBOL && tokens[i].value == ",") {
                i++;
            } else {
                throw std::runtime_error("Unexpected token in column list.");
            }
        }
    ...
    if (i < len && tokens[i].type == TokenType::KEYWORD && tokens[i].value == "WHERE") {
        ...
        while (i + 2 < len) {
            ...
            //optional AND/OR (not implemented here, could be added later)
            if (i < len && tokens[i].type == TokenType::SYMBOL && tokens[i].value == ";") {
                break;
            }
...
updateQuery Parser::parseUpdate(const std::vector<Token>& tokens){
    updateQuery q;
    size_t i=0;
    size_t len = tokens.size();

    if(tokens[i].type != TokenType::KEYWORD || tokens[i].value != "UPDATE"){
        throw std::runtime_error("Expected 'UPDTAE' Keyword!");
    }
    q.operation = tokens[i++].value;
    ...
    q.table = tokens[i++].value;

    if(i >= len || tokens[i].type != TokenType::KEYWORD || tokens[i].value != "SET"){
    ...
    if(i < len && tokens[i].type == TokenType::IDENTIFIER ){
        while(i+2 < len){
            ...
            if(i >= len || tokens[i].type != TokenType::OPERATOR){
                throw std::runtime_error("Expected an Assignment Operator!");
            }

            std::string op = tokens[i++].value;

            if (tokens[i].type != TokenType::STRING && tokens[i].type != TokenType::NUMBER && tokens[i].type != TokenType::BOOL) {
            throw std::runtime_error("Expected literal value!");
            }

            std::string rhs = tokens[i++].value;

            q.assignments.push_back({lhs,op,rhs});

            if(i<len && tokens[i].type == TokenType::KEYWORD && tokens[i].value == "AND"){
                i++;
                continue;
            }else{
                break;
        ...
        throw std::runtime_error("Expected a column name after 'SET'!");
    }
    ...
    if(i < len && tokens[i].type == TokenType::KEYWORD && tokens[i].value == "WHERE"){
        ...
        while(i+2 < len){
            if(i >= len || tokens[i].type != TokenType::IDENTIFIER){
                throw std::runtime_error("Expected a column name after 'WHERE'!");
            }
            std::string lhs = tokens[i++].value;
    ...
    if (i >= len || tokens[i].value != ";" || tokens[i].type != TokenType::SYMBOL) {
        throw std::runtime_error("Expected ';' at end of UPDATE statement.");
    }
...
showQuery Parser::parseShow(const std::vector<Token>& tokens){
    showQuery q;
    size_t i=0;

    if(tokens[i].type != TokenType::KEYWORD && tokens[i].value != "SHOW"){
        throw std::runtime_error("Expected 'SHOW' Keyword!");
    }

    q.operation = tokens[i].value;
    i++;

    if(tokens[i].type != TokenType::KEYWORD && tokens[i].value != "TABLES"){
        throw std::runtime_error("Expected 'TABLES' Keyword!");
        i++;
    }
    
// check why error while checking the symbol 

    // if(tokens[i].type != TokenType::SYMBOL && tokens[i].value != ";"){
    //     throw std::runtime_error("Expected ';' Symbol at the end of the Query!");
    // }
...
AST Parser::parser(std::string& query) {
...
// insertQuery Parser::parser(string& query){
...
//         } else if (command == "SELECT") {
//             selectQuery ast = parseSelect(tokens);
//             std::cout << "[SELECT] Table: " << ast.tableName << "\nColumns: ";
//             for (const auto& col : ast.columns) std::cout << col.value << " ";
//             std::cout << "\nWhere Clauses:\n";
//             for (const auto& cond : ast.whereClause) {
//                 std::cout << "  " << cond.column << " " << cond.op << " " << cond.valueToMatch << "\n";
//             }
//             // return ast;
...
//         //     std::cout << "Where Clauses:\n";
//         //     for (const auto& cond : ast.whereClause)
//         //         std::cout << "  " << cond.lhs << " " << cond.op << " " << cond.rhs << "\n";
...
//     selectQuery q = Parser::parser(query);
//     return 0;
// }
